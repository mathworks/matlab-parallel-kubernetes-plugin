...
if exitCode == 2
    iError(job, 'parallelexamples:GenericKubernetes:NoWritePermissionOnCluster', ...
        sprintf('Worker with user ID = %d, group ID = %d does not have permission to write to the job storage location on the cluster (path %s in PersistentVolumeClaim %s).', ...
...
elseif exitCode == 7
    iError(job, 'parallelexamples:GenericKubernetes:JobStorageNotFound', ...
        sprintf('Job storage location not found at path "%s" in PersistentVolumeClaim "%s".', ...
        cluster.AdditionalProperties.JobStoragePath, cluster.AdditionalProperties.JobStoragePVC));
end
end
